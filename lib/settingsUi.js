...
import { TOP_MARGIN_MIN, TOP_MARGIN_MAX } from './constants.js';
...
export function buildSettingsTab(host) {
    ...
    tab.add_child(buildSettingRow(host, 'Collapse', [
    ...
    host._marginValueLabel = new St.Label({
        text: `${clampMargin(host._settings.get_int('top-margin'))}`,
    ...
    minusBtn.connect('clicked', () => {
        host._settings.set_int('top-margin', clampMargin(host._settings.get_int('top-margin') - 2));
    });
    ...
    posRow.add_child(host._marginValueLabel);
...
export function syncSettingsUi(host) {
    ...
    const autoCollapse = host._settings.get_boolean('auto-collapse');
    const margin = clampMargin(host._settings.get_int('top-margin'));
    ...
    if (host._marginValueLabel)
        host._marginValueLabel.set_text(`${margin}`);
}
